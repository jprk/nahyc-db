...
|--------|---------|-------|-------|
| A | `Document` | Regulatorní/metodický dokument (metadata) | V01 + sloupce `identifier`, `jurisdikce`, `jurisdikce_uroven`, `file_path`, `needs_review`, `review_reason` |
...
| `needs_review` / `review_reason` | `BOOLEAN` NOT NULL DEFAULT `FALSE` / `VARCHAR(500)` NULL | **Doplněno 2026-09-11, uživatelský nález přes export (R2.5).** Export ukázal záznamy s nesmyslným titulkem/značkou a/nebo chybějícím popisem (zdroj: `parse_sinay_norms.py`'s souřadnicová PDF rekonstrukce rozdělí jeden zalomený řádek do špatných sloupců — např. `znacka` `"CEN/TC 326 Natural Gas Vehicles"` + `nazev_cz` `"- Fuelling and Operation"`). `src/tools/init_db.py`'s `detect_data_quality_issues()` PŘI IMPORTU zkontroluje: je `znacka` skutečné označení dokumentu, nebo jen odkaz na technickou komisi/holá edice-datová přípona (`is_garbled_znacka()`); vypadá titulek jako useknutý fragment textu (`is_fragment_title()` — vč. výjimky pro legitimně malým písmenem začínající česká/slovenská právní označení jako "zákon č. ..."); a chybí popis/anotace. Nikdy needhaduje/needopraví — jen označí a zapíše důvod(y) do `review_reason` (a do `data/incomplete_records_review_queue.json`, stejná konvence jako ostatní `*_review_queue.json`). `app/app.py`/`index.html` zobrazují viditelný příznak přímo v katalogu (na uživatelovo výslovné přání: záznam zůstává vidět, jen nese označení "Vyžaduje kontrolu" s `review_reason` jako tooltip) — nikdy se needstraní z exportu ani z vyhledávání. **Kořenová příčina obou "nesmyslná značka" případů opravena přímo v `parse_sinay_norms.py` (2026-09-11, viz `src/tools/0README.md`)** — `needs_review`/`is_garbled_znacka()` zůstávají jako obecná pojistka pro budoucí/jiné parsovací chyby stejného typu, ne jako trvalá záplata na tyto dva konkrétní případy. Ověřeno na reálném korpusu po opravě: 402/1183 dokumentů (400× chybějící popis, 7× fragment titulku, 0× nesmyslná značka). |
